...
public class NDEFMessages {
	...
	// ***********************************************************************/
	public static byte[] ConvertStringToNDEF_Text_ByteArray(
			String stringToConvert, DataDevice ma) {
		...
		if (PayloadLenght >= 255)
			PayloadLenght = 255;
		int maxMessageLenght = (Helper.ConvertStringToInt((ma.getMemorySize()
				.replace(" ", "")))) * 4;
		...
		if (stringToConvert.length() <= maxMessageLenght) {
			...
			strNDEFtobeWritten = "02656E" + strNDEFtobeWritten; // status byte +
																// language code

			int iNDEFtobeWrittenLen = (strNDEFtobeWritten.replace(" ", "")
					.length() / 2) - 1;
			if (iNDEFtobeWrittenLen < 0xFF) // 1 byte length
			{
				strNDEFlen = Helper
						.ConvertHexByteToString((byte) iNDEFtobeWrittenLen);
				boolLenOneByteRecord = true;
			} else {
				strNDEFlen = "0000"
						+ Helper.ConvertHexByteArrayToString(Helper
								.ConvertIntTo2bytesHexaFormat(iNDEFtobeWrittenLen)); // 4
																						// byte
																						// length
			...
			if (iNDEFlen < 0xFF) // 1 byte length
				...
				strNDEFlen = "FF"
						+ Helper.ConvertHexByteArrayToString(Helper
								.ConvertIntTo2bytesHexaFormat(iNDEFlen)); // 4
																			// byte
																			// length
																			// //FF
																			// +
			...
			strNDEFtobeWritten = Helper.ConvertHexByteToString(CC3)
					+ strNDEFtobeWritten;
			// CC2
			byte CC2 = (byte) 0x00;
			if (((memorySize + 1) * (blockSize + 1)) >= 2048) // memory size =
																// nb blocks *4
																// == nb bytes
				CC2 = (byte) 0xFF; // CC2
			else
				CC2 = (byte) (((memorySize + 1) * (blockSize + 1)) / 8); // CC2
			strNDEFtobeWritten = Helper.ConvertHexByteToString(CC2)
					+ strNDEFtobeWritten;
			// CC1
			...
			ConvertedString = Helper
					.ConvertStringToHexBytesArray(strNDEFtobeWritten);
		} else {
	...
	// * from magic number -> 0xE1 to Terminator TLV -> 0xFE
	// * Example : "http://www.jogtek.com" -> { 0XE1 ...................... 0XFE
	// }
	// ***********************************************************************/
	public static byte[] ConvertStringToNDEF_Url_ByteArray(
			String stringToConvert, DataDevice ma) {
		...
		if (stringToConvert.length() <= maxMessageLenght) {
			...
			if (stringToConvert.toLowerCase().startsWith("http://www.")) {
				strPrefixData = "01";
				strRecord = stringToConvert.toLowerCase().replace(
						"http://www.", "");
				...
				strRecord = stringToConvert.toLowerCase()
						.replace("mailto:", "");
			} else if (stringToConvert.toLowerCase().startsWith(
			...
			for (int i = 0; i < strRecord.length(); i++) {
				StringRecord += Helper
						.ConvertHexByteToString((byte) CharArray[i]); // Payload
			}

			strNDEFtobeWritten = "FE";
			strNDEFtobeWritten = StringRecord + strNDEFtobeWritten; // text
			strNDEFtobeWritten = strPrefixData + strNDEFtobeWritten; // prefix
																		// if
																		// any

			strNDEFtobeWritten = "55" + strNDEFtobeWritten; // status byte +
															// language code
			...
			strNDEFtobeWritten = "01" + strNDEFtobeWritten; // status byte +
															// language code
	...
	public static byte[] toUTF8(String str) throws Exception {
	...
	// ***********************************************************************/
	public static String ConvertNDEF_ByteArrayToString_Adapted(
			byte[] ByteArrayToConvert) throws Exception {
		...
		if (ByteArrayToConvert[intByteArrayCounter] == (byte) 0x03) {
			...
			do {
				...
				// If (iStructureRecord = TNF_WELL_KNOWN Or iStructureRecord =
				// TNF_ABSOLUTE_URI) Then 'NFC forum supported message
				if ((bytStructureRecord == 0x01)
						|| (bytStructureRecord == 0x03)) {
					int intNDEFtypeLength;
					intByteArrayCounter++;
					intLField--;
					// intNDEFtypeLength =
					// ByteArrayToConvert[intByteArrayCounter];
					...
					{
						ConvertedByteArray += "type:TEXT " + (char) 0x0D
								+ (char) 0x0A;
						...
						if (intNDEFTEXTstatusByte > 0) {
							for (int i = 0; i < intNDEFTEXTstatusByte; i++) {
								...
								// ByteArrayToConvert[intByteArrayCounter];
								// ConvertedByteArray +=
								// (char)ByteArrayToConvert[intByteArrayCounter];
							}
						}
						// NDEF text
						byte [] aa=new byte[intNDEFPayloadLength];
						for (int i = intNDEFTEXTstatusByte; i < intNDEFPayloadLength - 1; i++) {
							intByteArrayCounter++;
							intLField--;
							aa[i]=ByteArrayToConvert[intByteArrayCounter];
						}
						String ss;
						ss=utfToString(aa);
						ConvertedByteArray+=ss;
					} else if (intNDEFRecordType == 0x55) // Const RTD_URI =
															// "55"
					{
						ConvertedByteArray += "type:URI " + (char) 0x0D
								+ (char) 0x0A;
						// URI identifier
					...
					else if (intNDEFRecordType == 0x5370) // Const
															// RTD_SMART_POSTER
															// = "5370" ("Sp")
					{
					...
					else if (intNDEFRecordType == 0x6163) // Const
					...
					else if (intNDEFRecordType == 0x4863) // Const
					...
					else if (intNDEFRecordType == 0x4872) // Const
					...
					else if (intNDEFRecordType == 0x4873) // Const
															// RTD_HANDOVER_SELECT
															// = "4873" ("Hs")
					{
					...
					else if (intNDEFRecordType == 0x616374) // Const RTD_ACT =
															// "616374" ("act")
					{
						ConvertedByteArray += "type:ACT " + (char) 0x0D
								+ (char) 0x0A;
						// URI identifier
						int intNDEFUriIdentifier;
						intByteArrayCounter++;
						intLField--;
						intNDEFUriIdentifier = (int) (ByteArrayToConvert[intByteArrayCounter] & 0xFF);
						if (intNDEFUriIdentifier == 0x00) // Do the action
															// (Launch Browser /
															// send sms / make
															// the telphone call
															// ...)
							ConvertedByteArray += "   - do the action"
									+ (char) 0x0D + (char) 0x0A;
						else if (intNDEFUriIdentifier == 0x01) // Save action
																// for later
																// (copy uri in
																// url editor /
																// save sms /
																...
																// telephone
																// number)
							ConvertedByteArray += "   - save the action for later"
									+ (char) 0x0D + (char) 0x0A;
						else if (intNDEFUriIdentifier == 0x02) // Open for
																// editing (put
																// uri in
																// bookmark /
																// open sms in
																...
																// mais appelle
																// pas)
							ConvertedByteArray += "   - open the action"
									+ (char) 0x0D + (char) 0x0A;
						else
							// NFC forum = "RFU" can act as 0 or do nothing
							ConvertedByteArray += "   - action not understood"
									+ (char) 0x0D + (char) 0x0A;
					}
				...
				// ElseIf (iStructureRecord = TNF_MIME_MEDIA) Then 'NFC forum
				// MIME supported message
					...
					if (strNDEFRecordType.equals("text/x-vCard")) {
						...
						if (strMessage.equals("BEGIN:VCARD")) {
							...
							ConvertedByteArray += "type:MIME:"
									+ strNDEFRecordType + (char) 0x0D
									+ (char) 0x0A + (char) 0x0D + (char) 0x0A;
							intByteArrayCounter++;
							intLField--;
							String strMessageTemp = "";
							boolean boolNDEFmessage = false;
							while ((ByteArrayToConvert[intByteArrayCounter] != (byte) 0xFE)
									&& intLField > 0) {
								/*
								 * //String strMessageTemp = ""; if
								 * (ByteArrayToConvert[intByteArrayCounter] !=
								 * (byte)0x0D &&
								 * ByteArrayToConvert[intByteArrayCounter+1] !=
								 * (byte)0x0A) { if
								 * (ByteArrayToConvert[intByteArrayCounter] ==
								 * (byte)0x3B) //; strMessageTemp += " "; else
								 * strMessageTemp +=
								 * (char)ByteArrayToConvert[intByteArrayCounter
								 * ]; boolNDEFmessage=true; }
								 */
								// String strMessageTemp = "";
								if (ByteArrayToConvert[intByteArrayCounter] != (byte) 0x0D
										&& ByteArrayToConvert[intByteArrayCounter + 1] != (byte) 0x0A) {
									// if
									// (ByteArrayToConvert[intByteArrayCounter]
									// == (byte)0x3B) //;
									// strMessageTemp += " ";
									...
									if (boolNDEFmessage) {
										...
										// else if
										// (strMessageTemp.startsWith("FN"))
										// ConvertedByteArray +=
										// "FORMATED NAME : ";
										// else if
										// (strMessageTemp.startsWith("ORG"))
										// ConvertedByteArray +=
										// "ORGANISATION : ";
										// else if
										// (strMessageTemp.startsWith("TITLE"))
										// ConvertedByteArray += "TITLE : ";
										// else if
										// (strMessageTemp.startsWith("TEL"))
										// {
										// if
										// (strMessageTemp.contentEquals("FAX"))
										// ConvertedByteArray += "FAX : ";
										// else
										// ConvertedByteArray += "TEL : ";
										// }
										// else if
										// (strMessageTemp.startsWith("ADR"))
										// ConvertedByteArray += "ADDRESS :";
										// else if
										// (strMessageTemp.startsWith("LABEL"))
										// ConvertedByteArray += "LABEL :";
										// else if
										// (strMessageTemp.startsWith("URL"))
										// ConvertedByteArray += "URL : ";
										// else if
										// (strMessageTemp.startsWith("EMAIL"))
										...
										if (boolTextFound == true) {
											for (int i = 1; i < strMessageTemp
												...
												{
													indexStart = i + 1;
													i = strMessageTemp.length() + 100; // rajout�?5/05/2012
												}
											}
											String strMessageTempTemp = strMessageTemp
													.substring(indexStart,
															strMessageTemp
																	.length());
											ConvertedByteArray += strMessageTempTemp
													.replace("=0D=0A=", "")
													+ (char) 0x0D + (char) 0x0A;
										}
										strMessageTemp = "";
							...
							// End of While loop due to FE found (or intLField<0
							// but not yet managed)
							boolLastTLVblock = true;
						}
					} else if (strNDEFRecordType.startsWith("application/")) {
						ConvertedByteArray += "type:MIME:" + strNDEFRecordType
								+ (char) 0x0D + (char) 0x0A + (char) 0x0D
								+ (char) 0x0A;
						ConvertedByteArray += strNDEFRecordType + (char) 0x0D
								+ (char) 0x0A;
						...
						// End of While loop due to FE found (or intLField<0 but
						// not yet managed)
						boolLastTLVblock = true;
				...
				// ElseIf (iStructureRecord = TNF_EXTERNAL_TYPE) Then 'NFC forum
				// MIME supported message
					...
					ConvertedByteArray += "type:EXTERNAL:" + strNDEFRecordType
							+ (char) 0x0D + (char) 0x0A + (char) 0x0D
							+ (char) 0x0A;
					// NDEF text
				...
				if (ByteArrayToConvert[intByteArrayCounter + 1] == (byte) 0xFE)
					boolLastTLVblock = true;
			} while (intLField > 0 && inttimeout < 5
					&& boolLastTLVblock == false); // boolLastTLVblock=0xFE :
													// "Last TLV block in the
													// data area
		}
		// TLV
		else if (ByteArrayToConvert[4] == (byte) 0xFE) {
			inttimeout = 0; // rien de pr?�u pour l'instant
